...
- **Respect data licenses.** Before adding a derived pack, confirm you may
...
1. **Data labelers** — add labelled images to `datasets/training/` following
   the YOLO layout (`images/{train,val}` ↔ `labels/{train,val}`). Each label
   file: `class_id x_center y_center width height` (normalised 0–1).
...
3. **Tooling engineers** — harden the aggregators (COCO/BDD100K), add
